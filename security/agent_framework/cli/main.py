...
def parse_args() -> argparse.Namespace:
    ...
    run_parser.add_argument(
        "--submit",
        action="store_true",
        help="После локального сканирования отправить результаты в менеджер песочниц.",
    )
    run_parser.add_argument(
        "--output",
    ...
    if args.command == "run":
        if not getattr(args, "targets", None):
            raise SystemExit("Не заданы цели (--target или targets в конфиге).")
...
def handle_run(args: argparse.Namespace) -> int:
    ...
    print("⚙️  Preparing run submission:")
    ...
    try:
        ...
        print(f"❌ Failed to submit run to sandbox manager: {exc}")
        return 1

    print("✅ Run accepted by sandbox manager:")
...
def handle_list_modules() -> int:
    ...
    print("📦 Available profiles and modules:")
...
def handle_report(args: argparse.Namespace) -> int:
    ...
    print(f"📝 Reports subsystem not yet implemented. Requested run: {run_id}")
    return 0
...
def handle_preset(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"❌ Пресет '{args.name}' не найден.")
        return 1
    ...
    print(f"✅ Пресет '{args.name}' скопирован в {destination_path}")
    return 0
...
def render_human(results: List[dict]) -> None:
    for entry in results:
        ...
        if entry["findings"]:
            ...
            table.add_row("-", "Нет обнаруженных проблем", "", "")
...
def handle_local_run(args: argparse.Namespace, spec: RunSpec, run_id: str) -> int:
    ...
    if args.dry_run:
        print("🧪 Dry-run mode (local). Execution skipped.")
    ...
    if args.submit:
        try:
            ...
            response.raise_for_status()
            print("📨 Run registered in sandbox manager.")
        except httpx.HTTPError as exc:
            print(f"❌ Failed to register run in sandbox manager: {exc}")
            return 1
    ...
    if args.markdown:
        ...
        print(f"🗒️  Markdown-отчёт сохранён в {markdown_path}")
    if args.html:
    ...
    if args.knowledge_base:
        ...
        print(f"📚 Добавлено в knowledge base: {kb_path}")
    ...
    if args.publish_dir:
        ...
        print(f"🚀 Отчёты опубликованы в {publish_dir}")
    else:
    ...
    if args.s3_bucket and html_publish_path and md_publish_path:
        try:
            ...
            print("☁️  Отчёты загружены в S3/MinIO.")
    ...
    if args.confluence_url and html_content:
        ...
        if missing:
            print(f"⚠️  Confluence параметры не заданы: {', '.join(missing)}. Публикация пропущена.")
        else:
            try:
                ...
                print(f"📄 Отчёт опубликован в Confluence: {confluence_link}")
            except ConfluencePublishError as exc:
                print(f"❌ Ошибка публикации в Confluence: {exc}")
                return 1
    ...
    if args.slack_webhook:
        try:
            ...
            print(f"❌ Ошибка отправки Slack уведомления: {exc}")
            return 1
    ...
    if args.neo4j_url:
        if not (args.neo4j_user and args.neo4j_password):
            ...
            try:
                ...
                print("🗃️  Результаты отправлены в Neo4j.")
            except Neo4jSyncError as exc:
                print(f"❌ Ошибка синхронизации с Neo4j: {exc}")
                return 1
    ...
    if args.tickets_dir:
        ...
        if tickets:
            ...
            print(f"🎫 Сформированы тикеты: {tickets_path}")

            if args.ticket_webhook:
                try:
                    send_ticket_webhook(webhook_url=args.ticket_webhook, tickets=tickets)
                    print("📨 Тикеты отправлены через webhook.")
                except Exception as exc:  # pragma: no cover - network failure
                    print(f"❌ Ошибка отправки тикетов через webhook: {exc}")
                    return 1
    ...
    if args.submit:
        try:
            ...
            response.raise_for_status()
            print("📤 Results submitted to sandbox manager.")
        except httpx.HTTPError as exc:
            print(f"❌ Failed to submit results to sandbox manager: {exc}")
            return 1
...
def build_slack_summary(
    ...
    *,
    ...
    lines = [
        ...
        f"*Max severity*: {['🟢','🟡','🟠','🔴'][max_severity] if max_severity < 4 else '🔴'}",
        " | ".join(
    ...
    for entry in payload.get("results", []):
        for finding in entry.get("findings", []):
            top_findings.append(
                f"{severity_emojis.get(finding.get('severity', '').lower(), '•')} "
                f"{finding.get('title')} ({entry.get('target')})"
            )
    if top_findings:
        lines.append("*Findings:*")
        lines.extend(f"• {line}" for line in top_findings[:5])
