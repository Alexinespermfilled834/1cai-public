...
def train_model(self, config: Dict[str, Any]):
    ...
    task_logger.info(
        "Начато обучение модели", extra={"model_name": config.get("model_name")}
    )
...
def retrain_all_models():
    ...
    try:
        ...
        performance_summary = model_trainer.metrics_collector.get_performance_summary(
            hours_back=24
        )
...
def update_feature_store():
    ...
    try:
        ...
        recent_metrics = model_trainer.metrics_collector.get_performance_summary(
            hours_back=1
        )
                ...
                model_trainer.mlflow_manager.log_metrics(
                    {
                        ...
                        "roles_tracked": len(
                            [k for k, v in recent_metrics.items() if v]
                        ),
                    }
                )
    ...
    try:
        ...
        for experiment in experiments:
            ...
            if hasattr(experiment, "creation_time"):
                age_days = (
                    datetime.utcnow().timestamp() - experiment.creation_time / 1000
                ) / (24 * 3600)

                if age_days > 30:
                    try:
                        model_trainer.mlflow_manager.client.delete_experiment(
        ...
        task_logger.info(
            "Очистка завершена", extra={"experiments_deleted": cleanup_count}
        )
...
def check_model_drift():
    ...
    try:
        ...
        current_metrics = model_trainer.metrics_collector.get_performance_summary(
            hours_back=1
        )
        historical_metrics = model_trainer.metrics_collector.get_performance_summary(
            hours_back=168
        )  # Неделя назад
        ...
        for role in current_metrics:
            for metric_type in current_metrics[role]:
                ...
                historical_mean = (
                    historical_metrics.get(role, {}).get(metric_type, {}).get("mean", 0)
                )

                if historical_mean > 0:
                    drift_percent = (
                        abs(current_mean - historical_mean) / historical_mean * 100
                    )
        ...
        if drift_detected:
            task_logger.warning(
                "Обнаружен дрейф моделей", extra={"drift_cases": len(drift_details)}
            )
    ...
    try:
        ...
        for role, metrics in performance_summary.items():
            for metric_type, metric_data in metrics.items():
                ...
                if (
                    "accuracy" in metric_type.lower()
                    or "precision" in metric_type.lower()
                ):
                    threshold = 0.7
...
def process_metrics_batch(self, metrics_data: List[Dict[str, Any]]):
    ...
    task_logger.info(
        "Обработка пакета метрик", extra={"records_count": len(metrics_data)}
    )
            ...
            try:
                ...
                loop.run_until_complete(
                    model_trainer.metrics_collector.record_requirement_analysis_accuracy(
                        ...
                        predicted_requirements=metric_data.get(
