...
class MLPredictor(ABC):
    ...
    def __init__(
        ...
        logger.info("Инициализирован MLPredictor", extra={"model_name": model_name})
    ...
    def evaluate(
        ...
        logger.info(
            "Оценка модели", extra={"model_name": self.model_name, "metrics": metrics}
        )
...
class SklearnPredictor(MLPredictor):
    ...
    def predict(
        ...
        try:
            ...
            logger.debug(
                "Выполнено предсказание", extra={"samples_count": len(X_processed)}
            )
    ...
    def predict_proba(self, X: Union[pd.DataFrame, np.ndarray]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.debug(
                "Вычислены вероятности", extra={"samples_count": len(X_processed)}
            )
...
class TensorFlowPredictor(MLPredictor):
    ...
    def __init__(
        ...
        logger.info(
            "Инициализирован TensorFlowPredictor", extra={"model_name": model_name}
        )
    ...
    def fit(
        ...
        try:
            ...
            if isinstance(X, pd.DataFrame):
                ...
                X_processed = (
                    pd.DataFrame(X, columns=self.features)
                    .fillna(0)
