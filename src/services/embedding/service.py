...
class EmbeddingService:
    ...
    def encode(self, text: Union[str, List[str]], batch_size: int = 32, show_progress: bool = False, use_device: Optional[str] = None) -> Union[List[float], List[List[float]]]:
        if not text: return [] if isinstance(text, list) else []
    ...
    def _encode_single(self, text, batch_size, show_progress, use_device):
        ...
        try:
            ...
            start = time.time()
            embeddings = model.encode(text, batch_size=batch_size, show_progress_bar=show_progress, convert_to_numpy=True)
    ...
    def _encode_hybrid(self, text: List[str], batch_size: int, show_progress: bool) -> List[List[float]]:
        ...
        with self._executor:
            futures = {}
            if batch_cpu:
                futures[self._executor.submit(self._encode_single, batch_cpu, batch_size, show_progress, "cpu")] = (0, "cpu")
            if batch_gpu:
                futures[self._executor.submit(self._encode_single, batch_gpu, batch_size, show_progress, "gpu")] = (mid, "gpu")
