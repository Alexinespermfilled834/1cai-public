...
class OCRService:
    ...
    def _init_deepseek(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Loading DeepSeek-OCR model (attempt {attempt + 1}/{max_retries})...")
    ...
    def _init_fallback_providers(self):
        if self.provider != OCRProvider.CHANDRA_HF:
            try: self._init_chandra()
            except Exception: pass
        
        if self.provider != OCRProvider.TESSERACT:
            try: self._init_tesseract()
            except Exception: pass
    ...
    async def _try_fallback_providers(self, image_path: str, **kwargs) -> Optional[Dict[str, Any]]:
        fallback_attempts = []
    ...
    async def _chandra_ocr(self, image_path: str, max_tokens: int = 8192, include_images: bool = False) -> Dict[str, Any]:
    ...
    async def _tesseract_ocr(self, image_path: str) -> Dict[str, Any]:
        ...
        def _sync_process():
            ...
            return " ".join(text_parts), sum(confidences) / len(confidences) if confidences else 0

        loop = asyncio.get_event_loop()
        full_text, avg_confidence = await loop.run_in_executor(self._executor, _sync_process)
    ...
    async def _parse_structure_with_ai(self, text: str, document_type: DocumentType) -> Dict[str, Any]:
        try:
            ...
            result = await orchestrator.process_query(
                prompt,
                context={"type": "document_parsing", "document_type": document_type.value}
            )
