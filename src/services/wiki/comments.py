...
class WikiCommentsService:
    ...
    async def add_comment(self, page_id: str, author_id: str, content: str, parent_id: Optional[str] = None) -> CommentDTO:
        ...
        async with get_db_connection() as conn:
            ...
            """, comment_id, page_id, author_id, content, created_at, parent_id)
    ...
    async def resolve_comment(self, comment_id: str, resolved: bool = True):
        ...
        async with get_db_connection() as conn:
            await conn.execute("UPDATE wiki_comments SET resolved = $1 WHERE id = $2", resolved, comment_id)
