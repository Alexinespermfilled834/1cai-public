...
from sqlalchemy import Column, String, Integer, Boolean, DateTime, ForeignKey, Text, UniqueConstraint, JSON
...
class WikiNamespace(Base):
    ...
    path = Column(String, unique=True, nullable=False, index=True)  # e.g., "project/module"
...
class WikiPage(Base):
    ...
    revisions = relationship("WikiRevision", foreign_keys="[WikiRevision.page_id]", back_populates="page")
    current_revision = relationship("WikiRevision", foreign_keys=[current_revision_id], post_update=True)
