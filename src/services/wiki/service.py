...
class WikiService:
    ...
    async def get_page(self, slug: str, version: Optional[int] = None) -> Optional[PageDTO]:
        """
        ...
        async with get_db_connection() as conn:
            ...
            html_content = self.renderer.render(row['content']) if row['content'] else ""
    ...
    async def create_page(self, data: WikiPageCreate, author_id: str, blueprint_id: Optional[str] = None) -> PageDTO:
        """
        ...
        async with get_db_connection() as conn:
            async with conn.transaction():
                ...
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5, 1)
                """, page_id, namespace_id, data.slug, data.title, revision_id)
                
                # 2. Create Revision
                await conn.execute("""
                    INSERT INTO wiki_revisions (id, page_id, version, content, commit_message, author_id)
                    VALUES ($1, $2, 1, $3, $4, $5)
                """, revision_id, page_id, content, data.commit_message, author_id)
    ...
    async def update_page(self, slug: str, data: WikiPageUpdate, author_id: str) -> PageDTO:
        """
        ...
        async with get_db_connection() as conn:
            async with conn.transaction():
                # 1. Get current page
                page = await conn.fetchrow("SELECT id, version, title FROM wiki_pages WHERE slug = $1 FOR UPDATE", slug)
                if not page:
                    raise ValueError("Page not found")
                
                # 2. Optimistic Locking
                if page['version'] != data.version:
                    raise ValueError(f"Conflict: Page has been modified (v{page['version']} vs v{data.version})")
                ...
                await conn.execute("""
                    INSERT INTO wiki_revisions (id, page_id, version, content, commit_message, author_id)
                    VALUES ($1, $2, $3, $4, $5, $6)
                """, revision_id, page['id'], new_version, data.content, data.commit_message, author_id)
                ...
                await conn.execute("""
                    ...
                    WHERE id = $3
                """, new_version, revision_id, page['id'])
