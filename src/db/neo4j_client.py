...
class Neo4jClient:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"Failed to connect to Neo4j (attempt {attempt + 1}): {e}")
    ...
    def execute_query(self, cypher: str, parameters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        if "DELETE" in cypher.upper() or "DETACH" in cypher.upper() or "DROP" in cypher.upper():
             # Only allow read-only queries through this generic interface? 
             # For now, let's just log a warning if it looks destructive.
             logger.warning("Destructive Cypher query detected", extra={"cypher_preview": cypher[:50]})
        ...
        try:
            ...
            logger.error(f"Error executing Cypher query: {e}", extra={"cypher": cypher}, exc_info=True)
            raise
