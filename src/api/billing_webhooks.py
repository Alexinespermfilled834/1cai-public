# [NEXUS IDENTITY] ID: -4814217284133170667 | DATE: 2025-11-19
...
class BillingWebhookHandler:
        ...
        async with self.db.acquire() as conn:
            ...
            if tenant:
                ...
                await self._log_billing_event(
                    tenant["id"], "subscription_created", event
                )
    ...
    async def _handle_subscription_updated(self, subscription: Dict, event: Dict):
        ...
        async with self.db.acquire() as conn:
            ...
            if tenant:
                ...
                await self._log_billing_event(
                    tenant["id"], "subscription_updated", event
                )
    ...
    async def _handle_subscription_deleted(self, subscription: Dict, event: Dict):
        ...
        async with self.db.acquire() as conn:
            ...
            if tenant:
                ...
                await self._log_billing_event(
                    tenant["id"], "subscription_cancelled", event
                )
    ...
    async def _handle_payment_succeeded(self, invoice: Dict, event: Dict):
        ...
        async with self.db.acquire() as conn:
            ...
            if tenant:
                await self._log_billing_event(
                    tenant["id"], "payment_succeeded", event, amount_cents=amount
                )
    ...
    async def _log_billing_event(
        self, tenant_id, event_type: str, event: Dict, amount_cents: int = None
    ):
