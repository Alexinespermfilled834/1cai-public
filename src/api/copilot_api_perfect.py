# [NEXUS IDENTITY] ID: -9081243139180689587 | DATE: 2025-11-19
                    ...
                    suggestions.append(
                        {
                            ...
                            "score": 1.0
                            - (temp * 0.2),  # Higher temp = lower confidence
                        }
                    )
...
async def get_completions(api_request: Request, request: CompletionRequest):
    ...
    try:
        ...
        logger.error(
            "Timeout in get_completions", extra={"code_length": len(request.code)}
        )
...
async def generate_code(api_request: Request, request: GenerationRequest):
    ...
    if not isinstance(request.prompt, str) or not request.prompt.strip():
        logger.warning(
            ...
            extra={
                "prompt_type": type(request.prompt).__name__ if request.prompt else None
            },
        )
    ...
    if not isinstance(request.type, str) or request.type not in [
        "function",
        "procedure",
        "test",
    ]:
        logger.warning(
            "Invalid code type in generate_code", extra={"code_type": request.type}
        )
    ...
    try:
        ...
        code = await asyncio.wait_for(
            copilot_service.generate_code(
                prompt=request.prompt, code_type=request.type
            ),
            timeout=60.0,
        )
...
async def optimize_code(request: OptimizationRequest):
    ...
    try:
        ...
        result = await asyncio.wait_for(
            copilot_service.optimize_code(code=request.code, language=request.language),
            timeout=30.0,
        )
        ...
        logger.error(
            "Timeout in optimize_code", extra={"code_length": len(request.code)}
        )
...
async def generate_tests_for_code(api_request: Request, request: GenerationRequest):
    ...
    try:
        ...
        tests = await asyncio.wait_for(
            copilot_service.generate_code(prompt=request.prompt, code_type="test"),
            timeout=60.0,
        )
