...
from src.services.llm_gateway import LLMGateway, LLMGatewayResponse
...
class TestLLMGatewayResilience:
    ...
    def gateway(self):
        ...
        mock_manager.get_active_provider.return_value = ProviderConfig(name="test-provider", enabled=True)
    ...
    async def test_fallback_chain(self, gateway):
        ...
        gateway.manager.get_fallback_chain.return_value = {"primary": "primary", "chain": ["secondary"]}
        gateway.manager.get_provider.side_effect = lambda name: p1 if name == "primary" else p2
        ...
        gateway.get_client = Mock(side_effect=lambda name: mock_client_fail if name == "primary" else mock_client_success)
