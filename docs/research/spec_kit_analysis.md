...
|-------------|------------------------|----------------|----------------|
| **Культура и управление** | Общие OKR Dev+Ops, trunk-based, blameless postmortem, on-call ротация | Частично: spec-driven workflow, конституция, но нет формальной on-call/OKR и постмортемов | Создать раздел в конституции по постмортемам; задокументировать on-call/дежурства; добавить “DevOps KPIs”; использовать `job_market_devops_analysis.md` для roadmap |
...
| **CI/CD и качество** | Ступенчатые пайплайны, build once → promote, smoke после деплоя, отчёты | CI покрывает тесты, docs lint, spec validation; релизы автоматизированы (`release.yml`); smoke-tests компилируют модули и дергают `/health`; есть `make smoke-up` для docker check; HTML/JUnit/Allure артефакты; Jenkins/GitLab pipelines (`infrastructure/jenkins/Jenkinsfile`, `infrastructure/gitlab/.gitlab-ci.yml`) | Подготовить smoke job с поднятием минимального сервиса в CI; внедрить Allure отчёты на другие пакеты |
| **Infrastructure as Code / GitOps** | Terraform/Ansible/Helm, GitOps, secrets management, rollback playbook | Docker-compose + Make; теперь есть kind кластер (`infrastructure/kind`), Helm chart и Terraform (`infrastructure/terraform`), Ansible bootstrap (`infrastructure/ansible`); GitOps с Argo CD (`infrastructure/argocd`, `scripts/gitops/*`); rollback playbook в `docs/research/release_playbook.md` | Подготовить ADR по GitOps; сформировать rollback playbook; проверить секреты (Vault/ENV) |
| **Observability & Monitoring** | RED/USE метрики, логи, трассировки, SLO/Error Budgets, alert runbooks | `github-monitor.yml`, `dora-metrics.yml`, `/metrics` (Prometheus Instrumentator), локальный стек (`make observability-up`), Kubernetes стек (`make helm-observability`, Helm chart `infrastructure/helm/observability-stack` с Prometheus/Loki/Tempo/Grafana/OTEL), CI `observability-test.yml`, Telegram алерты (`telegram-alert.yaml`), Alertmanager (`observability/alertmanager.yml` + `observability/alerts.yml`), `docs/observability/SLO.md`, `docs/runbooks/alert_slo_runbook.md`; TODO: реальный alert канал (prod) | Добавить сбор SLI (Prometheus exporter/Logs), завести продовый alert-канал |
| **Resilience & DR** | Chaos engineering, резервирование, регулярные restore, документированные DR Playbooks | Бэкап-скрипты (`backup-restore.sh`), Litmus pod-delete (`infrastructure/chaos/litmus`, `docs/ops/chaos_engineering.md`), Istio профиль (`infrastructure/service-mesh/istio`, `docs/ops/service_mesh.md`); нет регулярных restore/chaos roadmaps | План геймдэя; добавить пункт “restore rehearsal” в TODO; зафиксировать процедуру |
| **Security (DevSecOps)** | Shift-left (SAST/DAST), dependency & secret scanning, policy-as-code | Bandit, Safety, run_full_audit; workflows `secret-scan.yml` (Gitleaks) и `trufflehog.yml` (Trufflehog); Conftest Rego (`policy/kubernetes/*.rego`), Semgrep (`security/semgrep.yml`), Checkov/Trivy (`scripts/security/run_checkov.sh`), make `policy-check`, `preflight`; Vault best practices (`docs/ops/vault.md`, CSI пример, Vault Agent, AWS/Azure sync, Terraform `azure-keyvault`, тест `scripts/secrets/test_vault_sync.sh`) | Расширить конституцию пунктами по least privilege; рассмотреть OPA/policy-as-code для Terraform/GitOps |
| **Feedback & Continuous Improvement** | Метрики delivery (lead time, deploy freq, MTTR, CFR), UX feedback, DevEx | Скрипт `collect_dora.py`, workflow `dora-metrics.yml`, AWS/Azure FinOps отчёты и Budgets (`scripts/finops/*`, workflow `finops-report.yml`, Slack/Teams), preflight checklist (`scripts/checklists/preflight.sh`); ретроспективы не описаны | Добавить визуализацию/дашборд, включить DORA summary в README/ретро; описать feedback loop |
