...
| **CI/CD и качество** | Ступенчатые пайплайны, build once → promote, smoke после деплоя, отчёты | CI покрывает тесты, docs lint, spec validation; релизы автоматизированы (`release.yml`); smoke-tests и HTML/JUnit артефакты добавлены | Подготовить smoke job с поднятием минимального сервиса; внедрить Allure |
...
| **Observability & Monitoring** | RED/USE метрики, логи, трассировки, SLO/Error Budgets, alert runbooks | `github-monitor.yml`, `dora-metrics.yml`, `docs/observability/SLO.md`, `docs/runbooks/alert_slo_runbook.md`; пока без автоматического экспорта SLI | Добавить сбор SLI (Prometheus/Logs), завести alert-канал |
