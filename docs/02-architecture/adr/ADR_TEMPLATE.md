# ADR-TEMPLATE: [Краткое название решения]

> ⚠️ **ВНИМАНИЕ:** Этот файл описывает состояние на **3 ноября 2025**.  
> **Актуальная версия:** [../../architecture/ARCHITECTURE_CURRENT_STATE.md](../../architecture/ARCHITECTURE_CURRENT_STATE.md) (обновлено 6 ноября 2025)

**Date:** YYYY-MM-DD  
**Status:** proposed | accepted | deprecated | superseded  
**Tags:** integration, architecture, performance, etc.

---

## Context

[Опишите контекст принятия решения]

**Бизнес-контекст:**
- Какая бизнес-цель?
- Какие ограничения?

**Технический контекст:**
- Какая техническая ситуация?
- Какие существующие системы?

---

## Problem

[Четко сформулируйте проблему или вызов]

**Проблема:**
- Что не работает или чего не хватает?
- Какие риски если не решить?

**Требования:**
- Функциональные
- Нефункциональные (SLA, производительность, и т.д.)

---

## Considered Alternatives

### Alternative 1: [Название]

**Описание:**
[Краткое описание варианта]

**Pros:**
- ✅ Преимущество 1
- ✅ Преимущество 2
- ✅ Преимущество 3

**Cons:**
- ❌ Недостаток 1
- ❌ Недостаток 2

**Оценки:**
| Критерий | Score (1-10) |
|----------|--------------|
| Feasibility | 8 |
| Cost | 7 |
| Complexity | 6 |
| Performance | 9 |
| Maintainability | 7 |
| **TOTAL** | **7.4** |

---

### Alternative 2: [Название]

**Описание:**
[Краткое описание варианта]

**Pros:**
- ✅ Преимущество 1
- ✅ Преимущество 2

**Cons:**
- ❌ Недостаток 1
- ❌ Недостаток 2

**Оценки:**
| Критерий | Score (1-10) |
|----------|--------------|
| Feasibility | 9 |
| Cost | 8 |
| Complexity | 9 |
| Performance | 6 |
| Maintainability | 8 |
| **TOTAL** | **8.0** |

---

## Decision

**Выбрано:** [Alternative X]

**Обоснование:**
[Почему выбрали именно этот вариант?]

**Ключевые факторы:**
1. [Фактор 1]
2. [Фактор 2]
3. [Фактор 3]

---

## Consequences

### Positive Consequences

- ✅ Последствие 1
- ✅ Последствие 2
- ✅ Последствие 3

### Negative Consequences

- ❌ Последствие 1 (Митигация: ...)
- ❌ Последствие 2 (Митигация: ...)

### Risks and Mitigation

| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| Risk 1 | High | Medium | Mitigation strategy |
| Risk 2 | Medium | High | Mitigation strategy |

---

## Implementation Plan

**Phase 1: [Название]** (Timeline: X weeks)
- Task 1
- Task 2

**Phase 2: [Название]** (Timeline: X weeks)
- Task 1
- Task 2

**Total Timeline:** X weeks

---

## Review and Update

**Review Date:** YYYY-MM-DD (обычно через 6 месяцев)

**Success Criteria:**
- [ ] Критерий 1
- [ ] Критерий 2
- [ ] Критерий 3

**Metrics to Track:**
- Метрика 1
- Метрика 2

---

## References

- [Link 1](url)
- [Link 2](url)
- Internal docs: ...

---

## Stakeholders

- **Decision Maker:** [Имя]
- **Consulted:** [Имя, Имя]
- **Informed:** [Команда]

---

**Approved by:** [Имя]  
**Approved date:** YYYY-MM-DD

